...
export default function CreateQuiz() {
    ...
    const handleTypeChange = (questionIndex, answerTypeValue) => {
        if (answerTypeValue === "single" || answerTypeValue === "multi") {
            ...
            for (var i = 0; i < questions[questionIndex].options.length; i++) {
                newOptions.push({option: questions[questionIndex].options[i].option, correctAnswer: i === 0})
            }
            setQuestions(prevQuestions => prevQuestions.map((question, key) =>
                key === questionIndex ? { ...question, options: newOptions, answerType: answerTypeValue } : question
            ))
        } else if (answerTypeValue === "connection") {

        }
    }

    const handleAddAnswer = (questionIndex) => {
        if (questions[questionIndex].answerType === "single" || questions[questionIndex].answerType === "multi") {
            setQuestions(prevQuestions => prevQuestions.map((question, key) =>
    ...
    const handleRemoveAnswer = (questionIndex, optionIndex) => {
        if (questions[questionIndex].answerType === "single") {
            setQuestions(prevQuestions => prevQuestions.map((question, key) => {
                if (key !== questionIndex) return question
                var updatedOptions = question.options.filter((_, optId) => optId !== optionIndex)
                if (question.options[optionIndex].answer) {
                    updatedOptions[0] = {option: updatedOptions[0].option}
                }
                return { ...question, options: updatedOptions }
            }))
        } else if (questions[questionIndex].answerType === "multi") {
            setQuestions(prevQuestions => prevQuestions.map((question, key) => {
                if (key !== questionIndex) return question
                    ...
                    <div className="w-full bg-white rounded shadow-sm p-5 mb-5">
                        ...
                        <div className="flex gap-3 mb-2">
                            ...
                            <label htmlFor="privateVisibility">
                                <input onChange={handleVisibilityChange} checked={visible} className="mr-1" type="radio" name="private" id="privateVisibility" value="private"/>
                                Private
                            </label>
                        </div>
                        {visible &&
